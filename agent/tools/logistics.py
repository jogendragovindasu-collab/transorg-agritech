...
def get_logistics_analysis(params: dict) -> dict:
    ...
    invalid_transit = int(transport_df.iloc[0]["total_trips"] - transport_df.iloc[0]["valid_transit_trips"]) if len(transport_df) > 0 else 1545
    ...
    return {
        "summary": f"Highest median transit: {longest_warehouse['destination_warehouse']} ({longest_warehouse['median_transit_hours']:.1f}h); Overall P90 long-transit threshold: {p90_threshold:.1f}h ({len(long_transit_warehouses)} warehouses exceed this).",
        "metrics": {
            "highest_warehouse": str(longest_warehouse["destination_warehouse"]),
            "highest_warehouse_median_hours": float(longest_warehouse["median_transit_hours"]),
        ...
        "evidence": f"Warehouse performance based on {int(longest_warehouse['trip_count'])} trips. P90 threshold ({p90_threshold:.1f}h) calculated from validated transport records; excludes {invalid_transit:,} invalid/negative transit records (total trips {int(transport_df.iloc[0]['total_trips'])} - valid transit trips {int(transport_df.iloc[0]['valid_transit_trips'])}).",
